...
def metric_card(title, value_id):
    return html.Div(
        ...
        children=[
            html.Div(title, style={"fontSize": "12px", "color": "var(--text-muted)", "marginBottom": "6px"}),
            html.Div(id=value_id, style={"fontSize": "22px", "fontWeight": "600", "color": "var(--text-primary)"}),
        ],
    )
...
def add_theme(fig, theme_mode: str = LIGHT_MODE, height=320, xangle=0):
    ...
    fig.update_layout(
        ...
        font=dict(family="Helvetica, Arial, sans-serif", color=palette["font"]),
        height=height,
    )
    fig.update_xaxes(tickangle=xangle, gridcolor=palette["grid"])
    fig.update_yaxes(gridcolor=palette["grid"])
    return fig


def get_platform_image_src() -> str:
    image_path = BASE_DIR / "assets" / "M1PQ-A.png"
    if image_path.exists():
        return "/assets/M1PQ-A.png"
    return (
        "data:image/svg+xml;utf8,"
        "<svg xmlns='http://www.w3.org/2000/svg' width='640' height='320'>"
        "<rect width='100%' height='100%' fill='%23E2E8F0'/>"
        "<text x='50%' y='50%' dominant-baseline='middle' text-anchor='middle' "
        "font-size='24' fill='%23475569' font-family='Arial'>M1PQ-A image not available</text>"
        "</svg>"
    )
...
app.layout = html.Div(
    id="app-root",
    className="dashboard-root theme-light",
    style={
                ...
                html.Div(
                    ...
                    children=[
                        html.Div(
                            ...
                            style={"flex": 1},
                            children=[
                                html.H2("Anode Trending Dashboard", style={"margin": 0, "color": "var(--text-primary)"}),
                                html.Div(
                                    ...
                                    style={"marginTop": "8px", "fontSize": "13px", "color": "var(--text-muted)"},
                                ),
                                html.Div(meta_txt, style={"marginTop": "4px", "fontSize": "13px", "color": "var(--text-muted)"}),
                                html.Div(anode_detail_txt, style={"marginTop": "4px", "fontSize": "13px", "color": "var(--text-muted)"}),
                            ],
                        ),
                        html.Div(
                            ...
                            children=[
                                html.Img(
                                    src=get_platform_image_src(),
                                    alt="M1PQ-A platform image",
                                    style={"width": "100%", "height": "120px", "objectFit": "cover", "borderRadius": "8px"},
                                ),
                                html.Div("M1PQ-A", style={"marginTop": "6px", "fontWeight": "600", "color": "var(--text-primary)"}),
                            ],
                        ),
                    ],
                ),
                    ...
                    children=[
                        html.Div(
                            ...
                            children=[
                                dcc.Graph(
                                    id="fig-performance",
                                    style={"backgroundColor": "var(--card-bg)", "borderRadius": "10px", "height": "320px"},
                                    config=NON3D_GRAPH_CONFIG,
                                ),
                                dcc.Graph(
                                    id="fig-depletion-year",
                                    style={"backgroundColor": "var(--card-bg)", "borderRadius": "10px", "height": "320px"},
                                    config=NON3D_GRAPH_CONFIG,
                                ),
                                dcc.Graph(
                        ...
                        html.Div(
                            ...
                            children=[
                                ...
                                dcc.Graph(id="fig-anode-3d", style={"height": "520px"}, config=ANODE_3D_GRAPH_CONFIG),
                            ],
                        ),
                        html.Div(
                            className="panel-card",
                            ...
                            children=[dcc.Graph(id="fig-retrofit", style={"height": "340px"}, config=NON3D_GRAPH_CONFIG)],
                        ),
                        html.Div(
                            className="panel-card",
                            style={
                            ...
                            children=[
                                html.H4("Anodes Depleting Within Projection", style={"margin": "0 0 8px 0", "color": "var(--text-primary)"}),
...
def update_dashboard(category, elevation_band, selected_stages, theme_mode):
    theme_mode = theme_mode if theme_mode in {LIGHT_MODE, DARK_MODE} else LIGHT_MODE
    ...
    fig_perf.add_trace(
        go.Scatter(
            ...
            mode="lines",
            line=dict(color=theme_palette(theme_mode)["faded"], width=2),
            name="Active (All)",
        )
    )
    fig_perf.add_trace(
        go.Scatter(
            ...
            mode="lines+markers",
            line=dict(color=theme_palette(theme_mode)["accent"], width=3),
    ...
    fig_perf = add_theme(fig_perf, theme_mode=theme_mode)
    ...
    fig_dep = add_theme(fig_dep, theme_mode=theme_mode)
    ...
    fig_elev = add_theme(fig_elev, theme_mode=theme_mode, xangle=45)
    ...
    fig_stage = add_theme(fig_stage, theme_mode=theme_mode)
    ...
    fig_3d.update_layout(
        ...
        paper_bgcolor=theme_palette(theme_mode)["paper"],
        plot_bgcolor=theme_palette(theme_mode)["plot"],
        template=theme_palette(theme_mode)["template"],
        font=dict(family="Helvetica, Arial, sans-serif", color=theme_palette(theme_mode)["font"]),
    )
    ...
    fig_retro = add_theme(fig_retro, theme_mode=theme_mode, height=340, xangle=45)
    ...
    return (
        f"dashboard-root {'theme-dark' if theme_mode == DARK_MODE else 'theme-light'}",
        fig_perf,
        fig_dep,
        fig_elev,
        fig_stage,
